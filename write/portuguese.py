...
class Portuguese(WriteWord):
    def __init__(self, how_many:str="") -> None:
        ...
        self._boards = int(how_many)
        self.not_exist = "empty"

        super().__init__(f"https://term.ooo/{how_many}", [(By.TAG_NAME, "wc-modal")])

    def run(self) -> None:
        for board in range(self._boards):

            self.rows = self.find_element(By.ID, f"board{board}").shadow_root.find_elements(By.CSS_SELECTOR, "[termo-row]")
            super()._run()

    def _write(self, row:WebElement, key:str) -> None: row.send_keys(key)
    def _get_all(self, row:WebElement, status:str) -> list: return row.shadow_root.find_elements(By.CLASS_NAME, status)
...
if "__main__" == __name__: Portuguese(argv[1]).run()